object Task:
  ...
  object UserService {
     ...
     * @return the user wrapped in `Some` if they exist, and `None` otherwise
     */
     ...
     *
     * @param name the name of the user
     * @return the user's grandchild wrapped in `Some` if they exist, and `None` otherwise
     */
    def getGrandchild(name: String): Option[User] =
      loadUser(name).flatMap(/* TODO */).flatMap(/* TODO */)
     ...
     */
    def getGrandchildAge(name: String): Option[Int] =
      /* TODO */
